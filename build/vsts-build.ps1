...
if ($AutoVersion) {
	...
	[version]$localVersion = (Import-PowerShellDataFile -Path "$($publishDir.FullName)\RTShell\RTShell.psd1").ModuleVersion
	Update-ModuleManifest -Path "$($publishDir.FullName)\RTShell\RTShell.psd1" -ModuleVersion "$($localVersion.Major).$($localVersion.Minor).$($newBuildNumber)"
}
